...
df = pd.read_excel('북한어(ㅈ,ㅉ,ㅊ,ㅋ).xlsx', sheet_name='Sheet1',engine="openpyxl")
                ...
                if df['suppos'][i+idx] == '무':
                    ...
                    if df['grp3rd'][i] == df['grp3rd'][i+idx]:
                        ...
                        try:
                            ...
                            df.loc[i,'explan']= df['explan'][i] + df['suppos'][i+idx]+df['explan'][i+idx]
                df = df.drop(i+idx)
                idx = idx+1
            df = df.reset_index(drop=True)
            i = i+idx
...
df.to_csv('210303북한어(ㅈ,ㅉ,ㅊ,ㅋ).csv',encoding='utf-8-sig')
